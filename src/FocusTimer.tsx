...
export default function FocusTimer({ timer, onUpdate, onSessionDone, breakMinutes }: Props) {
  ...
  // 頁面標題倒數，切到別的分頁也看得到
  useEffect(() => {
    document.title = `${fmtClock(remaining)} ${timer.phase === 'focus' ? '專注中' : '休息'} — 一程工作誌`
    return () => {
      document.title = '一程工作誌'
    }
  }, [remaining, timer.phase])

  // ---- 置頂浮窗（Document Picture-in-Picture）----
  // 開窗：記住「要自動跳」偏好（手動關窗會在 pip.ts 內記成關閉）
  const summonFloating = () => {
    setPipAuto(true)
    openFloating()
  }
  // 計時結束 / 元件卸載時關閉浮窗
      ...
      <div className={`zen ${timer.phase === 'break' ? 'zen-break' : ''}`}>
        ...
        <div className="zen-stage">
          <svg className="zen-enso" viewBox="0 0 400 400">
            ...
            {progress >= 0.999 && <circle cx="200" cy="200" r="26" fill="#3d5a73" className="zen-dot" />}
          </svg>
        ...
        <div className="zen-actions">
          ...
          {pipSupported() && (
            <button onClick={summonFloating} title="浮出置頂小窗，去別的分頁也看得到倒數">
              ⧉ 浮窗
            </button>
          )}
